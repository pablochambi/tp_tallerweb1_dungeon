...
public class ControladorReclutas {
    ...
    public ModelAndView reclutarHeroe(@PathVariable("id") Long idHeroe, HttpServletRequest request) {
        ...
        try {
            ...
            model.put(ATTR_CARRUAJE, carruaje);
            model.put(ATTR_USUARIO, usuario);
            model.put(ATTR_HEROES, heroesDespues);

            // Si era el último héroe, muestra mensaje
            if (heroesAntes.size() == 1 && heroesDespues.isEmpty()) {
                model.put("mensaje1", "¡Has reclutado al último héroe disponible!");
                model.put("mensaje2", "Vuelve la próxima semana para más héroes.");
            }
            ...
            model.put(ATTR_USUARIO, usuario);
            model.put("mensaje1", e.getMessage());
            return new ModelAndView(VISTA_CARRUAJE, model); // Mostrar directamente en lugar de redirigir
